...
func init() {
	if certFile, err := os.ReadFile(_cert_file); err == nil {
		...
		if _cert, err = x509.ParseCertificate(pemCert.Bytes); err != nil {
			logrus.Fatalf("Cannot parse timestamper certificate: %s", err)
		}
	} else {
		logrus.Fatalf("Cannot read timestamper certificate: %s", err)
	}

	if (_cert.KeyUsage & x509.KeyUsageDigitalSignature) == 0 {
		logrus.Fatalf("Timestamper certificate can't be used for signing (No `Digital Signature` in key usage)")
	}

	if !slices.Contains(_cert.ExtKeyUsage, x509.ExtKeyUsageTimeStamping) {
		logrus.Fatalf("Timestamper certificate can't be used for timestamping (No `Timestamping` in extended key usage)")
	}
	...
	if privFile, err := os.ReadFile(_priv_file); err == nil {
		...
		if _priv, ecParseError = x509.ParseECPrivateKey(pemPriv.Bytes); ecParseError != nil {
			if _priv, rsaParseError = x509.ParsePKCS1PrivateKey(pemPriv.Bytes); rsaParseError != nil {
				logrus.Fatalf("Cannot parse timestamper private key.\nEC: %s; RSA: %s", ecParseError, rsaParseError)
			}
		}
	} else {
		logrus.Fatalf("Cannot read timestamper private key: %s", err)
	}
	...
	if !pubKeyMatch {
		logrus.Fatalf("Public key in timestamper certificate does not match public key in provided private key")
	}
	...
	if caChainFile, err := os.ReadFile(_ca_chain_file); err == nil {
		for {
			...
			if caCert, err := x509.ParseCertificate(pemCert.Bytes); err == nil {
				if caCert.Equal(_cert) {
					logrus.Fatal("Timestamper certificate shall not be in CAs chain")
				}
			...
			if len(_ca_chain) > 1 {
				...
				if err := prevCa.CheckSignatureFrom(currCa); err != nil {
					logrus.Fatalf("Invalid CA chain: %s (%s; parent: %s)", err, prevCa.Subject, currCa.Subject)
				}
			}
		...
		logrus.Fatalf("Cannot read CA chain: %s", err)
	}

	if len(_ca_chain) == 0 {
		logrus.Fatal("Invalid CA chain: No CAs in chain")
	}

	if err := _cert.CheckSignatureFrom(_ca_chain[0]); err != nil {
		logrus.Fatalf("Timestamper certificate not been issued by CA in the chain (%s)", err)
	}
}
